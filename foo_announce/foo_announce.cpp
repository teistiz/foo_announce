...
static std::string escape_json_string(std::string str);
static std::string bake_json(const dict &dict);
...
// Writes a HTTP header into a destination buffer.
...
// Posts information about the song now playing.
// This is run in its own thread to avoid blocking the player.
DWORD WINAPI post_thread(LPVOID params) {
	...
	// found a host; let's bake a message and send it
...
// Converts a dictionary into JSON, escaping special characters as necessary.
// The incoming strings should be UTF-8 or compatible.
